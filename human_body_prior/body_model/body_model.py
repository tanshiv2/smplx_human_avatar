...
class BodyModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.model_type = {69: 'smpl', 162: 'smplx'}[njoints]

        assert self.model_type in ['smpl', 'smplx'], ValueError(
            'model_type should be in smpl or smplx.')
        ...
        # Regressor for joint locations given shape - 6890 x 24 for SMPL, 10475 x 24 for SMPL-X
        ...
        if 'pose_jaw' in params.keys():
            pose_jaw = params['pose_jaw']
        else:
            pose_jaw = torch.tensor(np.zeros((batch_size, 1 * 3 * 1)), dtype=dtype, requires_grad=True)
        self.register_parameter('pose_jaw', nn.Parameter(pose_jaw, requires_grad=True))

        if 'pose_eye' in params.keys():
            pose_eye = params['pose_eye']
        else:
            pose_eye = torch.tensor(np.zeros((batch_size, 1 * 3 * 2)), dtype=dtype, requires_grad=True)
    ...
    def forward(self, root_orient=None, pose_body=None, pose_hand=None, betas=None,
                trans=None, return_dict=False, v_template=None, clothed_v_template=None, pose_jaw=None, pose_eye=None,
                **kwargs):
        ''' Forward function
        ...
        verts, joints, joints_a_pose, bone_transforms, abs_bone_transforms, v_posed = lbs(betas=shape_components,
                                                                                          pose=full_pose,
